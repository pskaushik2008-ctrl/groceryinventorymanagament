...
function validateForm(quantity, price) {
...
function getFilteredAndSortedItems() {
  ...
  let list = [...items];
  if (searchValue) {
    list = list.filter((item) => String(item.id).includes(searchValue));
  }
...
itemForm.addEventListener("submit", (event) => {
  ...
  const validationError = validateForm(quantity, price);
  if (validationError) {
  ...
  if (id) {
    items = items.map((item) =>
      item.id === Number(id)
        ? { ...item, quantity: Number(quantity), price: Number(price), amount }
        : item
    );
